...
class CommentApiViewSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.admin_user = User.objects.create_user(username='admin', email='admin@example.com', avatar='pruebaIM.jpg',password=PASSWORD)
    ...
    def test_delete_comment_user(self):
        ...
        data_delete_comment = {
            'userId' : self.admin_user.pk,
            'commentId' : self.comment1.pk,
            'userToDeleteComment' : self.user1.pk
        }

        request = self.factory.delete(self.url3, data_delete_comment)
    ...
    def test_delete_comment_game(self):

        data_delete_comment = {
            'userId' : self.admin_user.pk,
            'commentId' : self.comment2.pk,
            'gameId' : self.game.pk
        }
